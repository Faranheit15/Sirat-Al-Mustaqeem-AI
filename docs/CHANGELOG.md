...
- Gemini embedding batches are now throttled with a 13 s inter-batch delay to stay under the free-tier ~5 RPM limit, eliminating `429 Too Many Requests` failures during ingestion. A `Retry-After`-aware exponential backoff (up to 3 retries, 60 s → 120 s → 240 s) handles any 429 that still slips through.
